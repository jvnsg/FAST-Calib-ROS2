...
struct Block
{
  ...
  std::vector<Eigen::Vector3d> lidar_pts; // 4 points
  std::vector<Eigen::Vector3d> qr_pts;   // 4 points
};

// Weighted SVD-based rigid transform estimation (lidar -> camera).
static RigidResult SolveRigidTransformWeighted(
    ...
    const std::vector<double> *weights = nullptr)
{
  ...
  out.R = R;
  out.t = t;
  out.rms = std::sqrt(rss / wsum);
  out.ok = true;
  return out;
}

// Parse a line like: "lidar_centers: {x,y,z} {x,y,z} ..."
static bool parseCentersLine(const std::string &line,
...
int main(int argc, char **argv)
{
  ...
  std::string record_path = params.output_path + "circle_center_record.txt";
  std::string out_path    = params.output_path + "multi_calib_result.txt";

  std::ifstream fin(record_path);
  if (!fin.is_open())
  {
    RCLCPP_ERROR(node->get_logger(), "Failed to open record file: %s", record_path.c_str());
    return 1;
  }
  ...
  if (lines.size() < 9)
  {
    RCLCPP_ERROR(node->get_logger(),
                 "Record file has fewer than 9 non-empty lines — need at least 3 scenes.");
    return 1;
  }

  // Parse blocks (3 lines each: time + lidar_centers + qr_centers)
  ...
  {
    RCLCPP_ERROR(node->get_logger(),
                 "Parsed %zu valid blocks, need at least 3. "
                 "Run single-scene calibration for 3+ different poses first.",
                 blocks.size());
    return 1;
  }

  RCLCPP_INFO(node->get_logger(), "Found %zu scene blocks. Using last 3.", blocks.size());

  // Use the last 3 blocks (12 point correspondences total)
  ...
  std::cout << "QR (camera) centers:" << std::endl;
  ...
  std::cout << BOLDYELLOW << "[Result] RMSE: " << BOLDRED << std::fixed
            << std::setprecision(4) << res.rms << " m" << RESET << std::endl;
  std::cout << BOLDYELLOW << "[Result] Multi-scene extrinsic parameters T_cam_lidar:" << RESET << std::endl;
  ...
  if (fout.is_open())
  {
    ...
    fout << "Pcl: [ "
         ...
         << std::setw(9) << res.t(2) << "]\n";
    fout.close();
    std::cout << BOLDYELLOW << "[Result] Multi-scene calibration results saved to "
              << BOLDWHITE << out_path << RESET << std::endl;
  }
  else
  {
    RCLCPP_WARN(node->get_logger(), "Failed to write results to: %s", out_path.c_str());
  }
